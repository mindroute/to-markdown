  ...
  return [
    ...
    ...(filteredDeclarations(declarations, omittedDeclarations, classNameFilter).flatMap(decl => {
      ...
      const nodes = [
        ...
        ...optionEnabled(omittedSections.fields) ? makeTable('Fields', [CELLS.NAME, options?.private === 'details' ? null : 'privacy', CELLS.TYPE, CELLS.DEFAULT, 'description', CELLS.INHERITANCE].filter(identity), fieldsDecl) : [],
        ...optionEnabled(omittedSections.methods) ? makeTable('Methods', [CELLS.NAME, options?.private === 'details' ? null : 'privacy', 'description', CELLS.PARAMETERS, CELLS.RETURN, CELLS.INHERITANCE].filter(identity), methodsDecl) : [],
